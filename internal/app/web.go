...
func (a *App) serveWeb(ctx context.Context, ready chan<- error) error {
	...
	mux.HandleFunc("/cfnat/clear-lists", ws.requireAuth(ws.handleCFNatClearLists))
...
func (w *webServer) handleCFNatConfig(rw http.ResponseWriter, r *http.Request) {
	...
	updates := map[string]string{
		...
		"availability_check_enabled":                         boolText(r.Form.Get("availability_check_enabled") == "on"),
		"check_url":                                          r.Form.Get("check_url"),
		"expected_status":                                    r.Form.Get("expected_status"),
		...
		"health_concurrency_enabled":                         boolText(r.Form.Get("health_concurrency_enabled") == "on"),
		"health_concurrency":                                 r.Form.Get("health_concurrency"),
		"recovery_probe_mode":                                r.Form.Get("recovery_probe_mode"),
		"recovery_concurrency_enabled":                       boolText(r.Form.Get("recovery_concurrency_enabled") == "on"),
		...
		"post_pool_speed_test_exempt_enabled":                boolText(r.Form.Get("post_pool_speed_test_exempt_enabled") == "on"),
...
func (w *webServer) handleCFNatClearLists(rw http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		w.redirect(rw, "请求方法无效。")
		return
	}
	if err := w.app.clearManagedIPLists(r.Context()); err != nil {
		w.redirect(rw, "清空名单失败："+err.Error())
		return
	}
	w.app.broadcastState()
	w.redirect(rw, "已清空 IP 黑名单、IP 入池免测速名单、IP 入池不免测速名单。")
}
...
function injectExemptSwitch(){const details=document.querySelector('[data-i18n="postPoolExemptList"]')?.closest('details');if(!details||details.querySelector('[name="post_pool_speed_test_exempt_enabled"]'))return;const label=document.createElement('label');label.style.display='block';label.style.margin='12px 0';const input=document.createElement('input');input.type='checkbox';input.name='post_pool_speed_test_exempt_enabled';input.style.width='auto';{{if .Config.PostPoolSpeedTest.ExemptEnabled}}input.checked=true;{{end}}const text=document.createElement('span');text.id='exempt-enabled-label';text.style.marginLeft='6px';label.append(input,text);details.insertBefore(label,details.children[1]);const update=()=>{const l=localStorage.getItem('cfnat_lang')||'zh-Hant';text.textContent=l==='en'?'Enable IP speed-test exemption':l==='zh-Hans'?'启用 IP 入池免测速功能':'啟用 IP 入池免測速功能'};update();document.querySelectorAll('[data-lang]').forEach(btn=>btn.addEventListener('click',()=>setTimeout(update,0)))}injectExemptSwitch();
function injectAvailabilitySettings(){const details=document.querySelector('[data-i18n="optHealth"]')?.closest('details');if(!details||details.querySelector('[name="availability_check_enabled"]'))return;const box=document.createElement('div');box.className='sub-config';box.style.margin='12px 0';const label=document.createElement('label');const toggle=document.createElement('input');toggle.type='checkbox';toggle.name='availability_check_enabled';toggle.style.width='auto';{{if .Config.AvailabilityCheckEnabled}}toggle.checked=true;{{end}}const title=document.createElement('span');title.style.marginLeft='6px';label.append(toggle,title);const grid=document.createElement('div');grid.className='grid';const urlWrap=document.createElement('div');const urlLabel=document.createElement('label');const urlInput=document.createElement('input');urlInput.name='check_url';urlInput.value={{printf "%q" .Config.CheckURL}};urlWrap.append(urlLabel,urlInput);const statusWrap=document.createElement('div');const statusLabel=document.createElement('label');const statusInput=document.createElement('input');statusInput.name='expected_status';statusInput.type='number';statusInput.min='100';statusInput.max='599';statusInput.value={{printf "%q" (printf "%d" .Config.ExpectedStatus)}};statusWrap.append(statusLabel,statusInput);grid.append(urlWrap,statusWrap);const hint=document.createElement('p');hint.className='hint';box.append(label,grid,hint);details.insertBefore(box,details.children[1]);const update=()=>{const l=localStorage.getItem('cfnat_lang')||'zh-Hant';const zh=l==='zh-Hans';title.textContent=l==='en'?'Enable independent HTTP/TLS availability validation':zh?'启用独立 HTTP/TLS 可用性验证':'啟用獨立 HTTP/TLS 可用性驗證';urlLabel.textContent=l==='en'?'Availability URL / business SNI':zh?'可用性验证 URL / 业务 SNI':'可用性驗證 URL / 業務 SNI';statusLabel.textContent=l==='en'?'Expected HTTP status':zh?'预期 HTTP 状态码':'預期 HTTP 狀態碼';hint.textContent=l==='en'?'Runs after latency probing and validates the configured URL, TLS/SNI certificate, and HTTP status without changing the measured latency.':zh?'在延迟检测通过后验证指定 URL、TLS/SNI 证书和 HTTP 状态，不改变延迟检测结果。':'在延遲檢測通過後驗證指定 URL、TLS/SNI 憑證和 HTTP 狀態，不改變延遲檢測結果。'};update();document.querySelectorAll('[data-lang]').forEach(btn=>btn.addEventListener('click',()=>setTimeout(update,0)))}injectAvailabilitySettings();
