...
func (w *webServer) handleCFNatConfig(rw http.ResponseWriter, r *http.Request) {
	...
	updates := map[string]string{
		...
		"blacklist_speed_test_interval":       r.Form.Get("blacklist_speed_test_interval") + "m",
	...
	for key, value := range updates {
		if key != "ip_blacklist" && key != "post_pool_speed_test_exempt_list" && (strings.TrimSpace(value) == "" || value == "ms" || value == "s" || value == "m") {
			continue
		}
...
func (w *webServer) render(rw http.ResponseWriter, msg string) {
	...
	data := map[string]any{
		...
		"BlacklistSpeedIntervalMins": int(cfg.BlacklistSpeedTest.Interval.Value().Minutes()),
