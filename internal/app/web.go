...
</div><p id="sum-shodan-error" class="hint">{{index (index .Payload "shodan") "error"}}</p><details><summary data-i18n="fullStatus">查看完整運行狀態明細</summary><pre id="cfnat-status">{{index (index .Payload "cfnat") "text"}}</pre></details><details><summary data-i18n="liveLog">查看即時日誌</summary><pre id="realtime-log"></pre></details></section>
...
function setText(id,v){const el=document.getElementById(id);if(el)el.textContent=v||''}function cls(id,v){const el=document.getElementById(id);if(!el)return;el.classList.remove('ok','warn','bad');const tx=(v||'').toString();if(tx.includes('运行')||tx.includes('運行')||tx.includes('完成')||tx.includes('同步')||tx.includes('空闲')||tx.includes('空閒')||tx.includes('中')||tx.includes('forward'))el.classList.add('ok');else if(tx.includes('错误')||tx.includes('錯誤')||tx.includes('失败')||tx.includes('失敗')||tx.includes('error'))el.classList.add('bad');else el.classList.add('warn')}function appendLog(d){const el=document.getElementById('realtime-log');if(!el)return;const line=new Date().toLocaleString()+' | cfnat='+d.summary.cfnat+' scan='+d.summary.scan+' dns='+d.summary.dns+' shodan='+d.shodan.state+' ip='+d.summary.primary_ip+'\n';el.textContent=(line+el.textContent).slice(0,12000)}function applyStatus(d){setText('sum-cfnat',d.summary.cfnat);setText('sum-scan',d.summary.scan);setText('sum-primary',d.summary.primary_ip);setText('sum-dns',d.summary.dns);setText('sum-shodan-state',d.shodan.state);setText('sum-shodan-ips',d.shodan.ips);setText('sum-shodan-error',d.shodan.error);setText('shodan-ips-card',d.shodan.ips);setText('shodan-success-card',d.shodan.last_success||t('notFetched'));setText('cfnat-status',d.cfnat.text);setText('proxy-state',d.cfnat.proxy);setText('proxy-toggle',d.cfnat.proxy_on?t('pauseProxy'):t('resumeProxy'));['sum-cfnat','sum-scan','sum-dns','sum-shodan-state'].forEach(id=>cls(id,document.getElementById(id)?.textContent));appendLog(d)}function openModal(id){const el=document.getElementById(id);if(el&&el.showModal)el.showModal()}function closeModal(id){const el=document.getElementById(id);if(el)el.close()}function connectEvents(){const live=document.getElementById('live-state');const es=new EventSource('/events');es.onopen=()=>{if(live){live.dataset.state='connected';live.textContent=t('connected')}};es.onerror=()=>{if(live){live.dataset.state='disconnected';live.textContent=t('disconnected')}};es.addEventListener('status',ev=>{try{applyStatus(JSON.parse(ev.data))}catch(e){}})}applyLang();connectEvents();
