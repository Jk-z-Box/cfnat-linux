...
- 可選入池後逐個測速篩選：掃描完成並熱更新轉發池後，對池內 IP 再逐個做一輪下載測速；測速狀態會顯示在 Web「即時狀態總覽」，低於指定 MB/s 或無速度的 IP 會立即從轉發池剔除，並可選即時加入 IP 黑名單。
...
- 當健康 IP 少於自訂門檻時，背景觸發整池重選；若新掃描池最佳延遲低於目前健康池，整池熱替換；若新掃描池較慢，保留目前健康 IP，並從新掃描結果中按延遲補齊到目標池大小。健康重選不會清空冷卻恢復池；自動掃描和分批復篩結果會過濾仍在冷卻恢復池中的 IP，避免繞過冷卻直接回池。恢復池內 IP 若恢復健康且延遲低於池內最慢 IP，會重新參與排序並替換最慢 IP。
...
- Shodan IP Panel 支援每個配置獨立開啟「定時獲取最新資料」，可選固定間隔、每日指定時間、每週指定時間或每月指定日期時間，並顯示上次/下次自動獲取時間。
...
```text
IP/CIDR 來源 → 候選生成 → TCP 初篩 → 分批下載測速 → 分批可配置探測複篩 → 批次通過先熱更新目標池 → 最終延遲排序 → 最低延遲優先 TCP 轉發
                                  ↓
                         Cloudflare DNS 同步
                                  ↓
                      延遲監控、健康剔除與重新優選
...
- 在 Shodan 配置中為目前 profile 設定定時獲取最新資料，可選固定間隔、每日、每週或每月指定時間；
...
若啟用入池後逐個測速篩選，完整掃描完成並熱更新轉發池後，會使用同一個 `speed_test.url` 對目前轉發池中的 IP 逐個測速，每個 IP 最多測試 `post_pool_speed_test.timeout`。Web「即時狀態總覽」會顯示測速狀態，例如測速中、目前進度、免測跳過數與已剔除數。速度低於 `post_pool_speed_test.min_mbps`、無速度或測速失敗的 IP 會在該 IP 測完後立即從轉發池剔除；若 `post_pool_speed_test.auto_blacklist=true`，被剔除的 IP 會立即寫入 `ip_blacklist`，後續掃描不再使用。測速達標的普通 IP 會即時加入 `post_pool_speed_test.exempt_list` 免測名單，後續入池後逐個測速會直接跳過，避免重複消耗時間。若 IP 位於 `post_pool_speed_test.force_test_list`，則即使同時在免測名單中，每次入池後仍會強制測速；不達標時會從轉發池剔除並遷移回 `ip_blacklist`。Cloudflare DNS 會在這輪入池後篩選完成後再按最終轉發池同步，避免把剛剔除的低速 IP 同步出去。
